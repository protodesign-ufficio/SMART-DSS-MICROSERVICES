...
from app.core.config import PERCORSI_SERVICE_URL, ENABLE_PERCORSI_DELEGATION


class PercorsiDelegationError(Exception):
    pass


def delegation_enabled() -> bool:
    return ENABLE_PERCORSI_DELEGATION
...
def _request_json(method: str, path: str, payload=None, timeout: float = 4.0):
    ...
    try:
        ...
        raise PercorsiDelegationError(str(exc)) from exc
...
def post_json(path: str, payload, timeout: float = 4.0):
